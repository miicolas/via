...
PROJECT_PATH="apps/via/via.xcodeproj"
SCHEME="via"
...
if [ "$EFFECTIVE_BUNDLE_ID" != "$DEPLOY_BUNDLE_ID" ]; then
  log "Bundle ID Xcode inattendu: $EFFECTIVE_BUNDLE_ID (attendu: $DEPLOY_BUNDLE_ID)"
  exit 1
fi
if [ -z "$EFFECTIVE_TEAM_ID" ]; then
  log "Team ID Apple absent du projet Xcode."
  exit 1
fi

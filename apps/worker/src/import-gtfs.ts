...
async function importTransitNetwork(gtfsPath: string) {
  ...
  await db.transaction(async (tx) => {
    ...
    /**
     * Station identity outlives the rolling 30-day GTFS window. Seasonal stops
     * stay here without an active `transit_stop_routes` row, which hides them
     * from current-network readers while preserving PMR, affluence, toilets,
     * exits and elevators keyed to the station.
     */
...
/** Every GTFS-owned or GTFS-updated table, in one list both maintenance steps share. */
...
async function runImport(path: string, force: boolean): Promise<GtfsImportResult> {
...
/**
 * The single seam for every local or remote GTFS import. It owns the feed hash,
 * the cross-process lock, the network transaction and every GTFS-derived table.
 * External station enrichments deliberately live behind their own CLIs.
 */
export async function importGtfsSnapshot(
  gtfsPath: string,
  force = false
): Promise<GtfsImportResult> {
  const importLock = await reserveGtfsImportLock();
  return runWithImportLock(importLock, () => runImport(gtfsPath, force));
}

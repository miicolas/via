...
final class TransportTests: XCTestCase {
    ...
    func testTransportMapsDocumentedStatusCodes() {
        XCTAssertEqual(APITransport.error(for: 401), .unauthorized)
        XCTAssertEqual(APITransport.error(for: 429), .rateLimited)
        XCTAssertEqual(APITransport.error(for: 503), .unavailable)
        XCTAssertEqual(APITransport.error(for: 500), .server(statusCode: 500))
    }
